...
from aiogram.utils.markdown import bold, italic, link, text
...
from config import (BINANCE_SMART_CHAIN_URL, BNB_ADDRESS, BUY, FERNET_KEY,
                    KUCOIN_TASK_NAME, PANCAKESWAP_FACTORY_ADDRESS,
                    PANCAKESWAP_ROUTER_ADDRESS, SELL, TELEGRAM_CHAT_ID)
from handlers.base import send_message
from models import TelegramGroupMember
from utils import all_same

from . import cg, cmc, coingecko_coin_lookup_cache, eth, logger
...
def swap_tokens(
    token: str, amount_to_spend: float, side: str, user: TelegramGroupMember
) -> str:
    """
    ...
    if web3.isConnected():
        ...
        try:
            if side == BUY:
                ...
                txn_hash = web3.toHex(
                    pancakeswap_wrapper.make_trade(
                        BNB_ADDRESS, token, amount_to_spend, user_address
                    )
                )
            else:
    ...
    if len(args) != 2:
        ...
        if user:
            ...
            reply = await loop.run_in_executor(
                executor, swap_tokens, args[0], float(args[1]), BUY, user
            )
        else:
    ...
    if len(args) != 2:
        ...
        if user:
            ...
            if 0 < percentage < 101:
                ...
                loop = asyncio.get_event_loop()
                reply = await loop.run_in_executor(
                    executor, swap_tokens, args[0], percentage_to_sell, SELL, user
                )
            else:
